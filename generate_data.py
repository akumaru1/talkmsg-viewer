...
def process_member_dir(root_path, current_path):
    """Scan a single member directory, write its JSON file, and return its members.json entry."""
    root_path    = pathlib.Path(root_path)
    current_path = pathlib.Path(current_path)
    path_parts   = current_path.relative_to(root_path).parts
    ...
    for base_name in sorted(base_names):
        ...
        # Accept .jpg / .jpeg / .png for images
        img_p = next((current_path / f"{base_name}{e}" for e in ('.jpg', '.jpeg', '.png')
                      if (current_path / f"{base_name}{e}").exists()), None)
        # Accept .mp4 / .mov / .aac / .m4a for video or voice
        vid_p = next((current_path / f"{base_name}{e}" for e in ('.mp4', '.mov', '.aac', '.m4a')
                      if (current_path / f"{base_name}{e}").exists()), None)
        ...
        rel_img = str(img_p.relative_to(root_path)) if img_p else None
        rel_vid = str(vid_p.relative_to(root_path)) if vid_p else None
    ...
    # Avatar
    avatar_p    = root_path / group_name / member_name / "avatar.png"
    rel_avatar  = str(avatar_p.relative_to(root_path)) if avatar_p.exists() else None
    avatar_mtime = int(avatar_p.stat().st_mtime) if avatar_p.exists() else None

    # Hub preview: prefer text, fall back to a type label for media-only messages
    if member_messages:
...
def generate_data():
    ...
    for root, dirs, files in os.walk(root_dir):
        ...
        parts = current_path.relative_to(root_dir).parts
        print(f"  Scanning: {parts[0]} / {parts[1]} …")
        entry = process_member_dir(pathlib.Path(root_dir), current_path)
